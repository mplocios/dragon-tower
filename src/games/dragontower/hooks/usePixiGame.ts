...
export function usePixiGame(
  ...
  const scaleCanvas = useCallback(() => {
    ...
    const viewEl = ((app as any).canvas ?? (app as any).view) as HTMLCanvasElement | undefined;
    if (!viewEl) return;

    // Works with any parent layout — does not require #game-panel
    const container = (
      wrap.closest('#game-panel') ??
      wrap.closest('[data-game-container]') ??
      wrap.parentElement
    ) as HTMLElement | null;
    ...
    if (isMobile) {
      ...
      const availH = ((container?.clientHeight ?? window.innerHeight) * 0.9) - 20;
      const maxW = 520;
