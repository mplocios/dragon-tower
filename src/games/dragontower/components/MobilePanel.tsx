...
const MobilePanel: React.FC<MobilePanelProps> = ({
  balance, bet, diff, gstate, curMult, curWin,
  onBetChange, onDiffChange, onPlayAction, onRandom,
}) => {
  ...
  return (
    <div id="mob-panel">
      ...
      <div className="m-tabs-row">
        ...
        <button className="m-tab" disabled>Auto</button>
      </div>
      ...
      <div className="m-diff-row">
        ...
        <div className="m-diff-r">
          <select className="m-diff-sel" value={diff} disabled={playing}
      ...
      <div className="m-bal-row">
        ...
        <button className="m-rnd" disabled={!playing} onClick={onRandom}>Random Pick</button>
      </div>
      ...
      <div className="m-main">
        <div className="m-bet-card">
          ...
          <div className="m-bet-inner">
            ...
            <input
              className="m-bet-inp" type="number" value={bet} min={0.01} step={0.01}
              disabled={playing}
              onChange={e => onBetChange(parseFloat(e.target.value) || 0)}
            />
            <div className="m-arrows">
              <button className="m-arr" disabled={playing} onClick={() => onBetChange(Math.max(0.01, parseFloat((bet * 2).toFixed(2))))}>▲</button>
              <button className="m-arr" disabled={playing} onClick={() => onBetChange(Math.max(0.01, parseFloat((bet * 0.5).toFixed(2))))}>▼</button>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          disabled={playing && !canCash}
          style={canCash ? {
            width: 74, height: 74, borderRadius: '50%',
            background: 'radial-gradient(circle at 36% 30%,#f4ff40,#c8d800 55%,#9aaa00)',
            border: 'none',
        ...
        >
          {canCash ? (
            <div style={{ textAlign: 'center', lineHeight: 1.15 }}>
              ...
              <div style={{ fontSize: 8, fontWeight: 800, color: '#1c2e00', fontFamily: 'Rajdhani,sans-serif' }}>
                {fmt(curWin)}
              </div>
            </div>
          ) : (
        ...
        <div className="m-profit-card">
          <span className="m-plbl">Total Profit <span className="m-pmult">({curMult.toFixed(2)}×)</span></span>
          <div className="m-pi">
